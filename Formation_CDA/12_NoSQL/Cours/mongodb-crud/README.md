...
Il est possible de mettre des options sur notre insertion via un autre objet, par exemple l'attribut `ordered` qui par défaut est à `true`. Si on le change, alors il est possible de ne pas éliminer les insertion car l'arrivée d'une insertion infructueuse n'aura plus d'incidance sur les suivante de par l'absence d'ordre:
