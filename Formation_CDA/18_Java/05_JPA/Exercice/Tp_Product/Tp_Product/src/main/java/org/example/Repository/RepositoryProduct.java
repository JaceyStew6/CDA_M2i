...
public class RepositoryProduct extends BaseRepository<Product> {
    ...
    public List<Product> findAllByType(String productClass) {
        switch (productClass){
            case "Food":
                return em.createQuery("select p from ProductFood p").getResultList();
            case "Elec":
                return em.createQuery("select p from ProductElectronic p").getResultList();
            case"Housing":
                return em.createQuery("select p from ProductHousing p").getResultList();
            default:
                return null;
        }
    }
